...
def get_gspread_client(uploaded_json_bytes: bytes = None):
    """
    优先使用上传的 JSON；若无，再尝试 st.secrets['gcp_service_account']。
    """
    info = None
    # 1) uploaded
    if uploaded_json_bytes:
        import json
        try:
            info = json.loads(uploaded_json_bytes.decode("utf-8"))
        except Exception as e:
            return None, f"上传的 JSON 无法解析：{e}"
    # 2) st.secrets
    if info is None:
        try:
            ...
            info = None
    if not info:
        return None, "未提供凭据。请在侧边栏上传 Service Account JSON，或在 Secrets 配置 gcp_service_account。"
    creds = Credentials.from_service_account_info(info, scopes=SCOPE)
    gc = gspread.authorize(creds)
    sa_email = info.get("client_email", "(unknown)")
    return (gc, sa_email), None
...
def detect_columns(df):
    ...
    if any(x is None for x in need):
        st.error("无法识别必要列：日期、代码、操作、数量、价格。")
        return None
...
# ====================== Sidebar：Google 配置 ======================
st.sidebar.header("🔐 Google 配置")
gsheet_url = st.sidebar.text_input("Google 表格链接（整份共享）", help="把这一份表共享给家人；示例：https://docs.google.com/spreadsheets/d/XXXX/edit#gid=0")
cred_file = st.sidebar.file_uploader("上传 Service Account JSON（或在 Secrets 配置）", type=["json"], help="如使用上传方式，无需本地 secrets.toml。")

client_pack, err = get_gspread_client(cred_file.read() if cred_file else None)
gc = None; sa_email = None
if client_pack:
    gc, sa_email = client_pack
    st.sidebar.success(f"已加载凭据：{sa_email}")
else:
    st.sidebar.info(err or "请上传 Service Account JSON，或配置 st.secrets。")

spread_id = get_spreadsheet_id_from_url(gsheet_url) if gsheet_url else None
if spread_id and gc:
    try:
        sh = gc.open_by_key(spread_id)
        st.sidebar.success("已连接 Google 表格")
        st.sidebar.caption("如提示无权限，请将该表共享给上面的 service account 邮箱。")
    except Exception as e:
        sh = None
        st.sidebar.error(f"无法打开表：{e}")
else:
    sh = None

# ====================== 🟢 步骤 1｜导入 Fidelity 数据 ======================
st.subheader("🟢 步骤 1｜导入 Fidelity 数据")
st.caption("选择数据来源：上传新的 Fidelity CSV 或从 Google 读取历史结果。")
...
elif src_choice == "从 Google 表格读取":
    if sh is None:
        st.info("请先在左侧连接 Google 表格。")
    else:
        if st.button("📥 读取 Google 中保存的 Fidelity 结果"):
            try:
...
if 'sh' not in globals() or sh is None:
    ...
    if st.button("💾 保存最新 Fidelity 数据到 Google（覆盖旧数据）"):
        try:
            ...
            totals_map = {} if (totals is None or not isinstance(totals, dict)) else totals
